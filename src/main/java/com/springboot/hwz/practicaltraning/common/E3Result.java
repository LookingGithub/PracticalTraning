package com.springboot.hwz.practicaltraning.common;
...
//    }
    ...
    public static E3Result formatToPojo(String jsonData, Class<?> clazz) {
        try {
            ...
            //将jsonData的String格式下的Jason数据 转化成正儿八经的Json格式的数据
            JsonNode jsonNode = MAPPER.readTree(jsonData);
            JsonNode data = jsonNode.get("data");  //具体的传入jsonNode的对象数据(如：传入user对象数据)
            Object obj = null;
            if (clazz != null) {
                if (data.isObject()) {
    ...
    /**
     * 没有object对象的转化，即只包装 status数据和msg数据
     * 
     * @param json
     * @return
     */
